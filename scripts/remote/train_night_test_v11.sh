...
REC_DIR="/srv/smart-center-data/runtime/door_recordings/door_day_20260405_204130"
DS_NIGHT_TEST="/srv/smart-center-data/runtime/door_recordings/door_day_20260405_204130_dataset_night_test"
...
echo "[1/6] build night test dataset"
"$PY" /srv/smart-center/current/scripts/build_dataset_from_recording.py \
  ...
  --dataset-root "$DS_NIGHT_TEST" \
  --refs-root /srv/smart-center-data/runtime/door_refs \
  --step 2 \
  --max-frames 1800

echo "[2/6] merge into v11 dataset"
"$PY" /srv/smart-center/current/scripts/merge_door_datasets.py \
  --output "$DS_V11" \
  --inputs "$DS_V10" "$DS_NIGHT_TEST"

echo "[3/6] sanitize images"
python3 - <<'PY'
from pathlib import Path
import cv2
import numpy as np

root = Path("/srv/smart-center-data/runtime/door_recordings/door_20260405_day_mix_dataset_v11_night_test")
checked = 0
removed = 0
for p in root.rglob("*"):
    if not p.is_file():
...
for split in ("train", "val"):
    for label in ("open", "closed"):
        d = root / split / label
        counts[f"{split}_{label}"] = sum(1 for x in d.glob("*") if x.is_file()) if d.exists() else 0
print({"checked": checked, "removed": removed, **counts})
PY

RUN="door_cls_daymix_v11_nighttest_$(date +%Y%m%d_%H%M)"
RUN_ROOT="/srv/smart-center-data/runtime/door_retrain_runs/$RUN"
OUT_MODEL="$RUN_ROOT/door_state_cls_v11.pt"
mkdir -p "$RUN_ROOT"
echo "$RUN" > /srv/smart-center-data/runtime/door_retrain_runs/current_v11_run.txt
...
src = Path("/srv/smart-center-data/runtime/door_recordings/door_20260405_day_mix_dataset_v11_night_test").resolve()
run_root = Path("/srv/smart-center-data/runtime/door_retrain_runs") / Path("/srv/smart-center-data/runtime/door_retrain_runs/current_v11_run.txt").read_text().strip()
out_path = run_root / "door_state_cls_v11.pt"
...
with tempfile.TemporaryDirectory(prefix="door_cls_train_v11_") as tmp:
    ds = Path(tmp) / "dataset"
    if ds.exists():
        shutil.rmtree(ds)
...
echo "[6/6] quick static verify 90s"
python3 - <<'PY'
import json
import time
import urllib.request
...
while time.time() < end:
    ...
    last = st
    time.sleep(0.25)
print({"total": total, "flip_count": flip, "last_state": last})
PY
