(function installSmartCenterSnmp(global) {
    ...
    const getDeviceStatusMeta = utils.getDeviceStatusMeta || (status => ({ level: status?.online ? 'online' : 'offline', chipClass: status?.online ? 'online' : 'error', text: status?.online ? '在线' : '离线', isOnlineLike: !!status?.online }));
    const getCardStateClass = utils.getCardStateClass || (meta => (!meta || meta.level === 'offline' ? 'offline' : (meta.level === 'stale' || meta.level === 'error' ? 'warning' : '')));
    const openDetailsState = state.openDetailsState || (state.openDetailsState = {});
    ...
    function renderPersistedSnmpDetails(deviceId, sectionKey, title, meta, bodyHtml, open = false, className = 'snmp-inline-details') {
            ...
            const isOpen = Object.prototype.hasOwnProperty.call(openDetailsState, stateKey) ? !!openDetailsState[stateKey] : !!open;
    ...
    function bindSnmpDetailToggles(scopeEl) {
            ...
            root.querySelectorAll('details[data-snmp-detail-key]').forEach(detail => {
                ...
                detail.addEventListener('toggle', () => {
                    ...
                    if (key) openDetailsState[key] = detail.open;
                });
            });
        }
    ...
    function renderSnmpSwitchPortPanels(deviceId, portRows) {
            ...
            return sections.map(section => {
                ...
                const isOpen = Object.prototype.hasOwnProperty.call(openDetailsState, detailStateKey)
                    ? !!openDetailsState[detailStateKey]
    ...
    const api = {
        ...
        renderSnmpMetricChips,
        renderSnmpInlineMetrics,
        renderSnmpMiniList,
        renderSnmpFocusPanel,
        renderSnmpInlineDetails,
        ...
        formatSnmpBytesText,
        getSnmpDiskSummary,
        getSnmpProtocolProfile,
        renderSnmpHealthPill,
        buildSnmpDeviceFactItems,
        buildSnmpPrimaryMetricItems,
        renderSnmpDevicePrimaryPanels,
        renderSnmpAdvancedDetails,
        buildSnmpMetricRows,
        renderSnmpInterfaceChips,
        renderSnmpPortPreview,
        getSnmpAlertLevel,
        renderSnmpFlowList,
        renderSnmpStorageList,
        renderSnmpCapacityHero,
        renderSnmpDiskHealthList,
        renderQnapDriveBayPanel,
        renderSnmpFanList,
        renderSnmpDiskIoList,
        renderSnmpHighlightPanel,
        renderSnmpSpotlightCards,
        renderSnmpSwitchSegments,
        renderSnmpVendorCards,
        getSnmpPortPanelSortValue,
        sortSnmpPortRows,
        renderSnmpSwitchPortPanels,
        renderSnmpHealthBanner,
        getSnmpDeviceIcon,
        renderSnmpSwitchSection,
        renderSnmpKpiCards,
        renderSnmpNasSection,
        renderSnmpRouterSection,
        renderSnmpRoleSection,
        renderSnmpCompactCard,
        buildDashboardSnmpMetricItems,
        renderDashboardSnmpCard,
        renderSnmpCard,
        renderSnmpDeviceDetailPage,
    };
