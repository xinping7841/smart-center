...
For Feishu or local-model natural-language query behavior, read `docs/QUERY_KNOWLEDGE_BASE.md` first, then `docs/LOCAL_MODEL_QUERY_INTENTS.jsonl` and `docs/LOCAL_MODEL_LEARNING.md`. They define the read-only API allowlist, forbidden control actions, intent routing map, seed examples, and the export/RAG learning workflow.
